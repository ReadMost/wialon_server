...
def main():
		...
		with open(PRESETS_PATH, 'r') as cf:
			...
			if 'presets' in conf and type(conf['presets']) is dict:
				...
				if len(LAST_PRESETS):
					load_preset = \
						prompt(
							dict(message='Load from preset', type='list', choices=['no', 'yes'], name='load_preset'))[
							'load_preset']
					if load_preset == 'yes':
						choosen_preset = \
							prompt(dict(message='Choose preset', type='list', choices=LAST_PRESETS,
							            name='choosen_preset'))[
								'choosen_preset']
	...
	if PRESET:
		...
		# ASKING PROTOCOL
		SETTINGS['protocol'] = prompt(dict(message='Protocol', type='list', choices=['TCP', 'UDP'], name='protocol'))[
			'protocol']

		# ASKING ENDPOINT
		endpoint = prompt(
			dict(message='Choose endpoint', type='list', choices=[ENDPOINTS[ep] + ' (' + ep + ')' for ep in ENDPOINTS], \
		...
		# ASKING SRC
		SETTINGS['track_src_type'] = \
			prompt(dict(message='Track source type', type='list', choices=['URL', 'File'], name='track_src_type'))[
				'track_src_type']
	...
	try:
		with open(PRESETS_PATH, 'w') as cf:
			...
			if not PRESET:
				save_to_preset = \
					prompt(dict(message='Save as preset', type='list', choices=['no', 'yes'], name='answer'))['answer']
				if save_to_preset == 'yes':
					preset_name = prompt(dict(message='New preset name', type='input', name='preset_name'))[
						'preset_name']
					if len(preset_name):
	...
	if PROTOCOL == 'TCP':
		...
		while True:
			for msg in msgs:
				request = '#SD#NA;NA;{};{};{};{};140;0;100;6'.format(msg[0], msg[1], msg[2], msg[3]).encode(
					'utf-8') + b'\r\n'
